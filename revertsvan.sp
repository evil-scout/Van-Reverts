...
#define PLUGIN_VERSION "1.3.0 Van"
...
public void OnPluginStart() {
	...
	ItemDefine("Gunboats", "gunboats", "Reverted to pre-engiupdate, -75% blast damage from rocket jumping");																									 
	...
	ItemDefine("Panic Attack", "panic", "Reverted to pre-inferno, unique firing mechanic, +30% faster firing speed", ITEM_FL_PICKABLE);																																
...
public Action TF2Items_OnGiveNamedItem(int client, char[] class, int index, Handle& item) {
	...
	if (
		...
		TF2Items_SetFlags(item1, (OVERRIDE_ATTRIBUTES|PRESERVE_ATTRIBUTES));
		TF2Items_SetNumAttributes(item1, 1);
		TF2Items_SetAttribute(item1, 0, 135, 0.25); // blast jump resistance
	}
	...
	if (
		...
		TF2Items_SetAttribute(item1, 0, 708, 1.0); // panic attack function
		TF2Items_SetAttribute(item1, 1, 710, 1.0); // slower firing speed
		TF2Items_SetAttribute(item1, 2, 709, 2.5); // spread increase as health lowers
		TF2Items_SetAttribute(item1, 3, 651, 0.5); // fire rate increase as health lowers
		TF2Items_SetAttribute(item1, 4, 424, 0.67); // clip size reduced
		TF2Items_SetAttribute(item1, 5, 97, 0.5); // reload time decreased
		TF2Items_SetAttribute(item1, 6, 6, 0.7); // firing rate increased
		TF2Items_SetAttribute(item1, 7, 809, 0.0); // fixed shot disabled
		TF2Items_SetAttribute(item1, 8, 808, 0.0); // successive shot disabled
		TF2Items_SetAttribute(item1, 9, 45, 1.0); // bullets per shot normalized
		TF2Items_SetAttribute(item1, 10, 1, 1.0); // damage normalized
	}
